...
// ── MAP INIT ──────────────────────────────────────────────────────────
function initMap() {
  leafletMap = L.map('map', { zoomControl: true }).setView([54.875, -1.92], 11);
  ...
  updateMapMarkers();
  L.geoJSON(CATCHMENT_BOUNDARY,{style:{color:'#f1c40f',weight:2.5,opacity:.9,fillColor:'#f1c40f',fillOpacity:.05}}).addTo(leafletMap);
}
  ...
  chart = new Chart(document.getElementById('cc').getContext('2d'), {
    ...
    options: {
      ...
      scales: {
        ...
        y: { type:logY?'logarithmic':'linear', ticks:{font:{family:'IBM Plex Mono,monospace',size:10},color:'#5a7a92'}, grid:{color:'#daeaf5'}, title:{display:true,text:ul(param),font:{family:'IBM Plex Mono,monospace',size:10},color:'#5a7a92'} }
      }
    }
  });
...
document.getElementById('db').addEventListener('click', () => {
  ...
  a.download=`derwent_wq_${document.getElementById('ps').value.replace(/\s+/g,'_')}.png`;
  a.click();
});
